                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                            <DialogTitle className="text-xl font-bold text-text-primary truncate">
                                Asignaciones · {template?.name ?? ''}
                            </DialogTitle>
                            <DialogDescription className="text-xs text-text-secondary">
                                {list.length === 0
                                    ? 'Esta plantilla aún no está asignada a nadie.'
                                    : `Quién tiene esta plantilla y desde cuándo.`}
                            </DialogDescription>
                        </div>
                    </div>
                    {list.length > 0 && (
                        <div className="flex items-center gap-2 mt-3 flex-wrap">
                            {counts.active > 0 && (
                                <Badge
                                    variant="outline"
                                    className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 bg-success/10 text-success border-success/20"
                                >
                            ...
                            {counts.future > 0 && (
                                <Badge
                                    variant="outline"
                                    className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 bg-primary/10 text-primary border-primary/20"
                                >
                                    {counts.future} futura{counts.future === 1 ? '' : 's'}
                                </Badge>
                            )}
                            {counts.expired > 0 && (
                                <Badge
                                    variant="outline"
                                    className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 bg-text-hint/10 text-text-hint border-divider"
                                >
                    ...
                    {list.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                            ...
                            <p className="text-sm text-text-hint">
                                Aún no se ha asignado a ningún usuario ni grupo.
                            </p>
                        </div>
                    ) : (
                        list.map((a) => {
                            ...
                            return (
                                <div
                                    key={a.key}
                                    className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl bg-surface-variant/30 border border-divider/40 hover:border-primary/30 transition-colors"
                                >
