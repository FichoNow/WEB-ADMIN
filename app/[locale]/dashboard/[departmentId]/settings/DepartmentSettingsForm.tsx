...
export default function DepartmentSettingsForm({ departmentId, initialName }: Props) {
  const t = useTranslations('departmentSettings.form')
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-8">
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-2 border-b border-divider/50 pb-3">
              <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-text-hint">{t('section')}</h3>
            </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-sm font-bold text-text-secondary ml-1">{t('nameLabel')}</FormLabel>
                  <FormControl>
                    <Input
                  ...
                  <FormMessage />
                  <p className="text-[11px] text-text-hint mt-2 ml-1">{t('nameHint')}</p>
                </FormItem>
              )}
            />
          </div>
