...
const statusIcons: Record<string, { icon: any, color: string, bg: string }> = {
  ...
  REJECTED: { icon: XCircle, color: "text-error", bg: "bg-error/10" },
  CANCELLED: { icon: Clock, color: "text-text-hint", bg: "bg-surface-variant" },
};
...
export default function RequestsClient({ requests, employees, groups, departmentId }: Props) {
  const t = useTranslations('requests.client')
  ...
  const GROUP_LABELS: Record<string, string> = {
    [GROUP_ALL]: t('allGroups'),
    [GROUP_NONE]: t('noGroup'),
  };

  const typeLabels: Record<string, string> = {
    VACATION: t('type.VACATION'),
    PERMISSION: t('type.PERMISSION'),
    SICK_LEAVE: t('type.SICK_LEAVE'),
    MEDICAL_APPOINTMENT: t('type.MEDICAL_APPOINTMENT'),
    DAY_OFF: t('type.DAY_OFF'),
  };
  ...
  function StatusBadge({ status }: { status: AdminRequestStatus }) {
    ...
    return (
      <Badge variant="outline" className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md gap-1.5 border-none ${config.bg} ${config.color}`}>
  ...
  function formatRange(request: AdminRequestListItem) {
    const fmt = (d: string) => new Date(d).toLocaleDateString(dateLocale, { day: '2-digit', month: 'short' });
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <Tabs value={statusFilter} onValueChange={(v) => setStatusFilter(v as RequestFilter)} className="w-full sm:w-fit min-w-0">
          <TabsList className="bg-surface border border-divider/50 p-1 rounded-xl w-full sm:w-auto grid grid-cols-2 sm:flex sm:h-11 h-auto gap-1 sm:gap-0">
            {filters.map((f) => (
              <TabsTrigger key={f.value} value={f.value} className="rounded-lg px-3 sm:px-5 py-2 sm:py-1.5 text-xs font-bold data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all min-w-0">
        ...
        <div className="flex items-center gap-2">
          <Select value={groupFilter} onValueChange={(v) => setGroupFilter(v ?? GROUP_ALL)}>
            ...
            <SelectContent className="rounded-xl border-divider">
              <SelectItem value={GROUP_ALL} className="text-xs">{t('allGroups')}</SelectItem>
              <SelectItem value={GROUP_NONE} className="text-xs">{t('noGroup')}</SelectItem>
              {groups.map((g) => (
      ...
      {filtered.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-24 px-4 bg-surface/30 border border-dashed border-divider rounded-[2.5rem]">
          ...
          <p className="text-sm text-text-hint font-medium">{t('empty')}</p>
        </motion.div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <AnimatePresence mode="popLayout">
            {filtered.map((request) => (
              <motion.div key={request.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={`group relative flex flex-col gap-3 px-4 sm:px-6 py-4 sm:py-5 rounded-[2rem] bg-surface border transition-all duration-300 ${request.status === 'PENDING' ? 'border-amber-500/30 shadow-[0_8px_30px_rgb(245,158,11,0.05)]' : 'border-divider/50 hover:border-primary/20 hover:bg-surface-variant/30'}`}>
                <div className="flex items-start gap-4">
                  <div className="flex flex-col items-center justify-center w-14 h-14 rounded-2xl bg-surface-variant/50 border border-divider/50 shrink-0">
                    <span className="text-[9px] font-black uppercase tracking-widest text-text-hint">
                      {new Date(request.start_date).toLocaleDateString(dateLocale, { month: 'short' })}
                    </span>
                  ...
                  <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    <h4 className="text-base font-bold text-text-primary leading-none truncate">{request.employee_name}</h4>
                ...
                {request.status === "PENDING" ? (
                  <div className="flex items-center gap-2 pt-1 border-t border-divider/30">
                    <Button variant="ghost" onClick={() => setReview({ request, mode: "reject" })} className="flex-1 rounded-xl h-9 text-xs font-bold bg-error/10 text-error hover:bg-error hover:text-white transition-colors">
                      {t('reject')}
                    </Button>
                    <Button onClick={() => setReview({ request, mode: "approve" })} className="flex-1 rounded-xl h-9 text-xs font-bold">
                      {t('approve')}
                    </Button>
                  </div>
                ) : (
