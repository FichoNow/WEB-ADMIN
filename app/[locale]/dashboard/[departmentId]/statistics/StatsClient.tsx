...
import { minutesToHours, buildChartData, calcTrend } from './stats-utils'
...
export default function StatsClient({ bundle, userStats, employees, projectsOverview, currentUserId, departmentId }: Props) {
  ...
  if (!bundle?.overview) {
    return <div className="flex items-center justify-center h-64 text-text-hint">{tStats('noData')}</div>
  }
  ...
  const isWeekly     = overview.period_label === tStats('thisWeek') || overview.period_label === 'Esta Semana'
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <PageHeader
        title={isProjectsTab ? t('titleProjects') : t('title')}
        description={
          isProjectsTab
            ? t('descProjects')
            : t('desc', { period: overview.period_label })
        }
        actions={
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <Tabs value={tab} onValueChange={(v) => {
          ...
          <TabsList className="bg-surface border border-divider/50 p-1 h-11 rounded-xl w-full sm:w-auto grid grid-cols-3 sm:flex">
            {(
              [
                { key: 'general',    label: t('tabGeneral'),    shortLabel: t('tabGeneralShort'),    Icon: LayoutDashboard },
                { key: 'individual', label: t('tabIndividual'), shortLabel: t('tabIndividualShort'), Icon: UserCircle2 },
                { key: 'proyectos',  label: t('tabProjects'),   shortLabel: t('tabProjectsShort'),   Icon: Folder },
              ] as const
          ...
          <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 w-full sm:w-auto">
            <Select value={isWeekly ? 'current' : String(currentMonth)} onValueChange={(v) => handleMonthChange(v ?? 'current')}>
              <SelectTrigger className="w-full sm:w-44 h-11 bg-surface border-divider/50 rounded-xl text-xs font-medium min-w-0">
                ...
                <SelectValue>{isWeekly ? t('thisWeek') : MONTH_LABELS[currentMonth - 1]}</SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            {isIndividualTab && (
              ...
              >
                <SelectTrigger className="col-span-2 w-full sm:w-52 h-11 bg-surface border-divider/50 rounded-xl text-xs font-medium min-w-0">
                  ...
                  <span className="truncate">{selectedUser?.name ?? t('allEmployees')}</span>
                </SelectTrigger>
                <SelectContent className="rounded-xl border-divider p-0 overflow-hidden" style={{ width: 240 }}>
                  ...
                  <div className="overflow-y-auto max-h-56 py-1">
                    {[{ id: 'all', name: t('allEmployees') }, ...employees]
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-text-primary">{t('insightsTitle')}</span>
                  </div>
