...
import { createProject } from "@/app/repositories/projects-repository"
import type { ProjectActionState } from "@/app/types/admin/action-states/project-state"

/**
 * Server Action para crear un proyecto desde el panel admin.
 * 
 * Recibe los datos del formulario, valida lo básico y llama al respository que a su vez llama a la API.
 */
export async function createProjectAction(
    ...
    try{
        ...
        return { success: t("projects.created") }
    }catch (err) {
        return {
            error: err instanceof Error ? err.message : t("errors.projectCreate"),
        }
    }
}